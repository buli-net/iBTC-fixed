...
class SettingsActivity : BaseNavActivity() {
    ...
    override fun onCreate(b: Bundle?) {
        ...
        findViewById<LinearLayout>(R.id.itemBackupSeed).setOnClickListener {
            val et = EditText(this).apply { inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD }
            AlertDialog.Builder(this).setTitle("Nhập mật khẩu")
                .setView(et)
                .setPositiveButton("Xem") { _, _ ->
                    if (et.text.toString() == prefs.getString("password", "")) {
                        val seed = prefs.getString("seed", "")
                        AlertDialog.Builder(this).setTitle("Seed phrase của bạn")
                            .setMessage(seed)
                            .setPositiveButton("Đã sao lưu", null).show()
                    } else {
