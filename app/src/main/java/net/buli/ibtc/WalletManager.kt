...
class WalletManager(private val ctx: Context) {
    ...
    fun init() {
        ...
        // ĐÚNG constructor cho 0.14: (params, directory, filePrefix)
        kit = object : WalletAppKit(params, File(ctx.filesDir, info.id), "ibtc") {
            override fun onSetupCompleted() {
                // 0.14 không có setAllowSpendingUnconfirmedTransactions()
                wallet().coinSelector = AllowUnconfirmedCoinSelector()
            }
        }.apply {
            setBlockingStartup(false)
            restoreWalletFromSeed(seed) // khôi phục từ seed
            startAsync()
            awaitRunning()
        }
    }
    ...
    fun send(to: String, amountBTC: Double, feeRateSatVb: Int): String {
        return try {
            ...
            w.completeTx(req)
            w.commitTx(req.tx)
            kit!!.peerGroup().broadcastTransaction(req.tx).broadcast().get()
