...
class WalletManager(private val ctx: Context) {
    ...
    fun hasWallets(): Boolean = prefs.all.isNotEmpty()
    ...
    fun getActive(): WalletInfo? {
        ...
        active = WalletInfo(
            id,
            prefs.getString("${id}_name", "") ?: "",
            prefs.getString("${id}_seed", "") ?: ""
        )
        return active
    }
    ...
    fun price(): Double {
        var txt = httpGet("https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT")
        var p = Regex("\"price\":\"([\\d.]+)\"").find(txt)?.groupValues?.get(1)?.toDoubleOrNull()
        if (p != null) return p
        txt = httpGet("https://blockchain.info/ticker")
        p = Regex("\"USD\"[^}]*\"last\":([\\d.]+)").find(txt)?.groupValues?.get(1)?.toDoubleOrNull()
        if (p != null) return p
        txt = httpGet("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd")
        p = Regex("\"usd\":([\\d.]+)").find(txt)?.groupValues?.get(1)?.toDoubleOrNull()
        return p ?: 0.0
    }
