...
class WalletManager(private val ctx: Context) {
    ...
    fun getActive(): WalletInfo? {
        return getAll().find { it.id == prefs.getString("active", null) }
    }
    ...
    fun getAddress(): String {
        return if (isReady()) wallet!!.currentReceiveAddress().toString() else ""
    }

    // Lấy số dư với fallback
    fun getBalance(): Double {
        ...
        for (base in balanceApis) {
            try {
                ...
                // Lưu cache
                prefs.edit().putString("cache_balance_$addr", balance.toString())
