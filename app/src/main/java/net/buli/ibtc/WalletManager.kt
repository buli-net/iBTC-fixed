...
class WalletManager(private val ctx: Context) {
    ...
    private val balanceApis = listOf(
        "https://blockstream.info/api/address/",      // API chính
        "https://mempool.space/api/address/",         // Dự phòng 1
        "https://blockchain.info/q/addressbalance/"   // Dự phòng 2
    )
    ...
    fun getBalance(): Double {
        ...
        for (base in balanceApis) {
            try {
                val url = if (base.contains("blockchain.info")) {
                    base + addr
                } else {
                    base + addr
                }
