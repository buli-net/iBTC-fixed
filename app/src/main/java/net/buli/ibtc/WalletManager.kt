...
class WalletManager(private val ctx:Context){
    ...
    fun createWallet(name:String):WalletInfo{
        ...
        val info=WalletInfo(id,name,seed.mnemonicCode!!.joinToString(" "), seed.derivationPath.toString())
        prefs.edit().putString("w_$id","$name|${info.seed}|${info.xpub}").putString("active",id).apply()
        kit?.stopAsync(); kit=null; init(); return info
    }
    fun getWallets():List<WalletInfo> = prefs.all.keys.filter{it.startsWith("w_")}.mapNotNull{ k-> prefs.getString(k,"")?.split("|")?.let{WalletInfo(k.removePrefix("w_"),it[0],it[1],it.getOrElse(2){""})}}
    fun getActive():WalletInfo? = prefs.getString("active",null)?.let{id-> prefs.getString("w_$id","")?.split("|")?.let{WalletInfo(id,it[0],it[1],it.getOrElse(2){""})}}
    ...
    fun getTransactions():List<TransactionInfo> = kit?.wallet()?.getTransactionsByTime()?.map{tx-> val v=tx.getValue(kit!!.wallet()); TransactionInfo(tx.txId.toString(), v.toBtc().toDouble(), tx.updateTime, if(v.isPositive)"Nhận" else "Gửi") }?.reversed()?:emptyList()

    private fun httpGet(u:String):String = try{ (URL(u).openConnection() as HttpURLConnection).apply{connectTimeout=5000;readTimeout=5000}.inputStream.bufferedReader().readText() }catch(_:Exception){""}
    ...
    fun price(): Double {
        ...
        for ((u, r) in urls) {
            ...
            if (p!= null && p > 1000) {
                if (p!= lastPrice) { lastPrice = p; prefs.edit().putFloat("last_price", p.toFloat()).apply() }
                return p
            }
        }
        return lastPrice
    ...
    fun send(to:String,amt:Double):String{ val r=Coin.parseCoin("%.8f".format(amt)); val tx=kit!!.wallet().createSend(Address.fromString(params,to),r); kit!!.wallet().commitTx(tx); kit!!.peerGroup().broadcastTransaction(tx).future().get(); return tx.txId.toString() }
}