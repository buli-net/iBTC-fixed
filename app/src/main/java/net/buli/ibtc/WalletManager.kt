...
class WalletManager(private val ctx:Context){
    ...
    fun newWallet():String{ val seed=DeterministicSeed(SecureRandom(),128,""); val m=seed.mnemonicCode!!.joinToString(" "); prefs.edit().putString("seed",m).apply(); kit?.stopAsync(); kit=null; init(); return m }
    ...
    fun txs()=kit?.wallet()?.getTransactionsByTime()?.map{val v=it.getValue(kit!!.wallet()); TxInfo(it.txId.toString(),v.toBtc().toDouble(),it.updateTime,if(v.isPositive)"Nhận" else "Gửi")}?.reversed()?:emptyList()
    fun price()=try{ Regex("[0-9]{5,}\\.[0-9]+").find(URL("https://blockchain.info/ticker").readText())?.value?.toDouble()?:0.0 }catch(_:Exception){0.0}
    fun send(to:String,amt:Double):String{ val tx=kit!!.wallet().createSend(Address.fromString(params,to),Coin.parseCoin("%.8f".format(amt))); kit!!.wallet().commitTx(tx); kit!!.peerGroup().broadcastTransaction(tx); return tx.txId.toString() }
}